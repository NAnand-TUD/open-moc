...
def writeUMG2out(stator,name,factor=1,scale=1,angle=180,ncurv=8):
	...
	if(len(PRESSURE)%factor==0): PRESSURE_C = np.zeros(((len(PRESSURE)//factor,3)))
	else: PRESSURE_C = np.zeros(((len(PRESSURE)//factor+1,3)))
	#pdb.set_trace()
	for i in range(len(PRESSURE)): 
		if(i%factor==0): PRESSURE_C[i//factor] = PRESSURE[i]
	PRESSURE_C[-1] = PRESSURE[-1]

	if(len(SUCTION)%factor==0): SUCTION_C = np.zeros(((len(SUCTION)//factor,3)))
	else: SUCTION_C = np.zeros(((len(SUCTION)//factor+1,3)))
	...
	#ncurv = 6
	if os.path.isdir('Db'):	filename = 'Db/geometry.'+name
	else: filename = 'geometry.'+name
	curvetype = '               \' S \'\n'
	dim_np = '                 dim                 np\n'
	val_dim_np = '                   2                  '
	x_y = '\n                   x                   y\n'
...
def writeBladeSpecs(RSS,ScaleNoz,name):
	...
	#f = open(name,'w')
		...
		f.write('# Throat coordinate points\nThrt_coords_up = [{},{},{},{}]\n'.format(RSS.thrtStrtUp[0],RSS.thrtStrtUp[1],RSS.thrtEndUp[0],RSS.thrtEndUp[1]))
		f.write('Thrt_coords_down = [{},{},{},{}]\n\n'.format(RSS.thrtStrtD[0],RSS.thrtStrtD[1],RSS.thrtEndD[0],RSS.thrtEndD[1]))
		...
		f.write('# Nozzle coordinate points\nNoz_coords_up = [{},{},{},{}]\n'.format(RSS.nozStrtUp[0],RSS.nozStrtUp[1],RSS.nozEndUp[0],RSS.nozEndUp[1]))
		f.write('Noz_coords_down = [{},{},{},{}]\n\n'.format(RSS.nozStrtD[0],RSS.nozStrtD[1],RSS.nozEndD[0],RSS.nozEndD[1]))
...
def RadiusLimitPlot(radii,colors,RSS,pitch,nBlades=-1):
	if (nBlades==-1): RadiusLimitPlot360(radii,colors)
	else:
		if (nBlades%2 == 0): 
